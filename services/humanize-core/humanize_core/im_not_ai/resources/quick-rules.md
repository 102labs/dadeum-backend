...
`humanize-monolith` 에이전트가 한 콜에서 탐지·윤문·자체검증을 끝내기 위해 사용하는 슬림 룰북. Strict 전용 `stric-rules.md`에서 fast path에 필요한 S1·S2 핵심 패턴만 추려 처방과 함께 한 줄로 압축했다.
...
| E-7 | 청자 경어법 4단계(해라/하게/하오/해요/합쇼) 일관성 손실 | S2 | 한 단락 내 혼용 금지, 원문 register 유지 |
...
| ID | 패턴 | 심각도 | 처방 |
|---|---|---|---|
| J-1 | 헤딩 마크다운 ** 강조 남발 | S2 | 업무 글에서는 거의 다 제거 |
| J-2 | 따옴표 강조 5회+ | S1 | 핵심 한두 개만 살리고 평어로 |
| J-3 | 불릿 리스트 과다 | S2 | 목록 의미가 약하면 문단 산문으로 통합 |
...
3. **문체 이탈 없음**: 원문 register와 업무 문맥을 유지했는가
...
> v2.0 신규/보강은 A-7·A-15·A-16·A-18·A-19·E-2·E-7·F-4 **8건 (A-17 hold)**. post-editese 3축 metric은 본 룰북 미반영(metric only 트랙). A-17 무정물·추상명사 '-들'은 학술 anchor(전영철 2007·곽은주·진실로 2011) 강하나 외부 회차(2026-05-07 위키 6편)에서 양성 0건 — NMT 원본 출력 회차 후 v2.1에서 동일 ID로 재평가.
