...
Rewrite 단계에서 사용하는 적극 윤문 룰북. 원문 전체를 한 번 자연스럽게 고치고, Audit/review 단계는 문체를 다시 평가하지 않고 잘못 바뀐 의미·수치·날짜·인용·고유명사만 복원한다.

원칙: 먼저 룰북을 적극 적용해 고친다. 원문을 그대로 복사하는 것은 윤문 실패다. 보존 대상 값은 그대로 두고, 나머지 업무 문장은 흐름·어순·리듬·명확성·번역투를 실제로 개선한다.
...
- Rewrite should apply the rulebook assertively because preservation audit runs afterward.
...
- A: S1 잔존 0건, S2 잔존 2건 이하, tone 설정 체감 가능, self-check 모두 통과.
