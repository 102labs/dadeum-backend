...
단일 strict rewrite rulebook. Rewrite 단계는 이 파일의 처방을 기준으로 원문 전체를 한 번 자연스럽게 윤문한다. Audit/review 단계는 문체를 다시 평가하지 않고, 잘못 바뀐 의미·수치·날짜·인용·고유명사만 복원한다.

원칙: fast처럼 실행하되 보존 감사가 뒤따른다. 원문을 그대로 복사하는 것은 윤문 실패다. exact-preserve 대상만 글자 단위로 유지하고, 나머지 업무 문장은 흐름·리듬·명확성·번역투를 실제로 개선한다.

## Protect

Do not edit: 고유명사, 제품명, 모델명, 기관명, 사람 이름, 숫자, 날짜, 시간, 단위, URL, 이메일, 코드, 법률·규정 조문, 수식, 직접 인용, 사용자가 지정한 protected_terms.

Do not flag: 의미 변화가 없는 어순·조사·접속어·문장 길이 조정, 스타일을 더 좋게 만들 수 있다는 일반 의견, exact-preserve 대상 주변의 안전한 문장 개선.
