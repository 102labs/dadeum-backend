...
Strict 단일 rewrite rulebook. Rewrite 단계는 이 파일을 기준으로 원문 전체를 한 번 자연스럽게 윤문하고, audit/review 단계는 이 파일의 보존 원칙을 기준으로 잘못 바뀐 의미·수치·인용·날짜·고유명사를 복원한다.
...
| C-7 | S2 | "먼저/반면/결국" 3단 공식 | 접속사를 줄이고 본문에 녹여 문단 흐름으로 연결 | 실제 대비·결론 표지는 유지 |
| C-8 | S2 | "A인가, B인가" 대칭 대구 반복 | 한 번만 살리고 나머지는 평서로 바꾼다 | 핵심 문제제기 1회는 유지 |
| C-9 | S2 | "1) 2) 3)" 숫자 괄호 인덱싱 | 산문이나 단순 줄바꿈으로 바꾼다 | 계약·절차 번호는 유지 |
| C-10 | S1 | 콜론 부제 헤딩 "X: Y", "X: A에서 B로" 반복 | 짧은 헤딩 또는 평서형 제목으로 정리 | 고유 제목 형식은 보존 |
...
- C-2 should not flatten useful checklists. Flag when bullets are shallow, repetitive, and could read better as a paragraph. Column/report bullet blocks may also be a J-style visual issue, but the actual case is covered here.
...
| D-5 | S1 | 기술·시대·시장·전략이 묻는다/요구한다/증명한다/질문을 던진다 | 사람·조직·상황 주어로 바꾼다 | 은유가 원문 핵심이면 유지 |
| D-6 | S1 | "~할 때입니다/~시점입니다/~할 순간입니다/~해야 할 때다/지금이야말로" 결말 공식 | 평서형 결론으로 닫거나 삭제한다 | 명확한 행동 촉구가 필요하면 약화 |
...
- D-3 covers list-introduction boilerplate and empty focus adverbs. Keep both as deletion-first cues.
- D-4 includes "막강한", "대대적", "강력한", "치열한", "뜨거운", "~의 가능성을 열어준다", "~의 새로운 장을 열다", "~시대가 도래했다". Lower hype, not the user's actual evaluation. If the original has evidence, keep the claim and make the wording more specific.
- D-5/D-6 preserve the business conclusion while removing formulaic drama. D-6 should usually close as a plain statement or be deleted when it adds only a call-to-action shell. For D-7, convert slogans like "'지식 전달자'에서 '학습 조력자'로" into direct sentences: "교사는 지식만 전달하지 않는다. 학생 곁에서 학습을 돕는다."
...
- G-1 covers both 추측·관측형 endings and future-default endings. Keep real forecasts; only reduce formulas that hide a present conclusion.
...
| ID | Sev | Signal | Fix | Keep |
|---|---|---|---|---|
| H-1 | S1 | 또한/따라서/즉/나아가/아울러/게다가/더욱이 등 문두 접속사 과다 | 대량 제거하고 문장 자체가 흐름을 잡게 한다 | 논리 전환이 필요한 곳은 유지 |
...
- H-1 is about density. Do not flag one necessary paragraph transition; flag a pattern of repeated paragraph-start connectors. When repeated, remove most connectors rather than replacing every one with another connector.
...
| J-2 | S1 | 개념어·강조어 따옴표 5회+ | 핵심 1~2개만 남긴다 | 직접 인용은 수정하지 않음 |
...
## Strict Review Self-Check

- 고유명사·제품명·모델명·기관명, 수치·날짜·단위, 직접 인용은 원문 대비 100% 보존한다.
- 원문 register를 보존한다. 격식체 원문을 평어체로 떨어뜨리거나, 대화체의 화자-청자 관계를 바꾸지 않는다.
- 잔존 S1 패턴은 0건을 목표로 한다. 보존 규칙 때문에 남긴 S1은 warnings 또는 unresolved reason에 남긴다.
- 원문에 없는 비유, 수사, 예시, 근거, 새 주장을 추가하지 않는다.

## Strict Quality Grade

- A: S1 잔존 0건, S2 잔존 2건 이하, 변경률 10~25%, self-check 모두 통과.
- B: S1 잔존 0건, S2 잔존 4건 이하, self-check 대부분 통과.
- C: S1 잔존 1~2건 또는 self-check 일부 미통과. review에서 보수 수정하거나 warnings에 남긴다.
- D: S1 잔존 3건 이상, 의미 보존 실패, 직접 인용·수치·고유명사 훼손, 또는 변경률 50% 초과와 다른 위험 신호가 동반된 경우. 원문에 가깝게 복원하거나 high-risk warning을 남긴다.
